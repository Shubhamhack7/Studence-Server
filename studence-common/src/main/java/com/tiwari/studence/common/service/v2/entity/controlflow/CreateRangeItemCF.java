...
import com.tiwari.studence.common.service.v2.requestBuilder.CreateRangeItemRequestBuilder;
...
public class CreateRangeItemCF<Pb extends GeneratedMessageV3>
        extends AControlFlow<CreateRangeItemCF.State, Pb, ErrorException> {
  enum State {
    CREATE_ITEM,
    DONE
  }
  ...
  private final CreateRangeItemRequestBuilder<?> m_requestBuilder;
  ...
  public CreateRangeItemCF(DynamoDbTableInfo<?> dbInfo,
          CreateRangeItemRequestBuilder<?> requestBuilder,
          SecondaryIndexGenerator<Pb> secondaryIndexGenerator, Pb entity, String id,
          String rangeKey) {
    super(State.CREATE_ITEM, State.DONE);
    m_dbInfo = dbInfo;
    m_entity = entity;
    ...
    m_id = id;
    m_rangeKey = rangeKey;
    addStateHandler(State.CREATE_ITEM, new UpdateStoreHandler());
  }

  private class UpdateStoreHandler implements StateHandler<State> {
    private IFuture<Void, ErrorException> m_createFuture;
    ...
    public void registerCalls() {
      ...
      PutItemRequest request = m_requestBuilder.createCreateItemRequest(
              dbClientAndtable.getSecond(), m_entity, m_id, m_rangeKey,
              m_secondaryIndexGenerator.getAllSecondaryIndexes(m_entity));
      m_createFuture = dbClientAndtable.getFirst().putItemAsync(request);
