...
import com.tiwari.studence.util.common.StudenceThreadManager;
...
public class StudenceManagementFilter implements Filter {
  ...
  public void init(FilterConfig filterConfig) throws ServletException {
   ...
   /* Thread schedulerThread = new Thread(() -> {
      try {
        new StudenceSchedulerModule();
      } catch (Exception e) {
        throw new LoggedRuntimeException("Error running Quartz scheduler:", e.getStackTrace());
      }
    });
    schedulerThread.start();*/
    StudenceThreadManager manager = new StudenceThreadManager(null);
    manager.setWaitForCompletion(StudenceSchedulerModule.class,
            false); // ClassB throws exception for concurrency
    manager.start();
    try {
      manager.submitTask(new StudenceSchedulerModule(), StudenceThreadManager.ThreadTypeEnum.NON_CONCURRENT);
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
    ...
    if (registration != null) {
      ...
      try {
        ...
        servlet.init(
                new StandardServletConfig(servletContext, registration.getName(), new HashMap<>()));
